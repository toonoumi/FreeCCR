...
**Cineon Log → Workspace is a DECODE, not a display transform**: the `cineon_log` checkbox runs **right after Master Gain** (inside `_apply_working_space_recovery` on a windowed base, in the single normalised pass on a non-windowed one, in the numpy pre-stage on OpenCL) and encodes into the **working space's curve (sRGB)** — not at the end of the chain into Rec.709 γ 2.2, which is where it shipped. Both follow from what the stage is for: everything below it (White Balance's flat multiply above all, then the contrast S-curve pivoting on 0.5, saturation's luma weighting, the Curves editor's 0–255 domain) is built for display-referred data and was grading log values instead. The transform **never applied a matrix** — it is transfer-function-only, the primaries are already the workspace's, so "→ Rec.709" only ever described the gamma. It is **floored at 0 but not ceilinged** (the sRGB power segment is undefined below 0; the headroom above white has to survive for the White Point recovery just below it), which is why the old cached uint16 LUT is gone and the math runs in float32. Consequence worth knowing: the decode turns a cast from a log OFFSET into a linear RATIO, and a cast that is mild in log can exceed WB's 1.4/0.6 = 2.33:1 span — a strong cast belongs in Channel Levels BEFORE the decode, where a per-channel shift is a density offset. `cineon_log` is appended at the END of the `adjust_image`/`adjust_image_opencl`/`_apply_working_space_recovery` signatures for the same positional reason as `balance_*`, and the OpenCL numpy pre-stage now runs when Balance is active **or** the flag is set (the kernel would otherwise apply Channel Levels after the decode). Area layers grade the already-decoded base. See `spec/cineon-display-transform.md`.
