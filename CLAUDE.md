...
**Converting with no anchors is allowed**: with neither a black point nor a reference frame, Convert runs a faithful port of **NamiColor**'s negative transform — `d = −log10(16·p) + 1.0` in density (log) space, `_namicolor_invert`, from `Wavechaser/NamiColor` `NamiColor_dev.c` 3.1 — and the user grades it with Channel Levels, which on that base is working in the same density space NamiColor's own sliders do. The `16` and `+1.0` are NamiColor's fixed constants (a base near half scale lands at ~Cineon black), so nothing is measured off the frame. Deliberately NOT ported: NamiColor's Adobe→Rec.2020 matrix (it is applied in-place sequentially upstream, so the G/B rows consume already-overwritten values — a real bug baked into that look, which a colour-managed app must not reintroduce) and the default-off "Fit to Cineon Base" lift. The negative decode is already linear Adobe RGB (`gamma=(1,1)`, `output_color=Adobe`), which is exactly the input the DCTL expects. Crucially this is **not** a new `conversion_inputs` mode — it is the existing `mode: "bw"` recipe with `bw: (None, None)`, which is what it actually is (the B/W-point pipeline with zero points sampled). That keeps ~10 replay/dispatch sites working untouched, since they all hand `ci["bw"]` straight back to the converter; only `apply_bwpoint_normalization` / `ccr_normalize_with_bwpoint` and the catalog's `_ci_to_json`/`_ci_from_json` (which would `list(None)`) needed None-awareness. No black point also means no sprocket mask and no film-stock slopes — both are anchor-relative and are skipped. The UI confirms first via `_confirm_no_anchor_convert`, suppressible with Settings → General → Conversion (`convert/warn_no_anchor`, default on). See `spec/no-anchor-convert.md`.
