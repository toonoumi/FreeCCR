...
**That neutral is averaged, not sampled once**: `fit_camera_matrix` estimates `wb_mult` from a *range* of neutral patches — the midtone band of the chart's grey ramp (`WB_L_RANGE`, L\* 24–64; rows H–I of an ISO 12641-2 wedge, ~GS16–GS22 of a classic strip) — instead of the single lightest neutral, whose noise/dust/sheen would otherwise ride into every frame. Each patch is corrected against its **reference Lab** rather than forced to equal-RGB: a printed wedge carries a systematic cast that averaging does not cancel. The lightest neutral stays the *anchor* (exposure and white-relative normalisation, cLUT residual); only the channel ratios come from the average. See `spec/wb-neutral-range-average.md`.
